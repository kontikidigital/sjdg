 ...
 * Sant Joan de Gracia.
 *
 * This file adds the required CSS to the front end to the Sant Joan de Gracia Theme.
 *
 ...
 */
add_action('wp_enqueue_scripts', 'sjdg_css');
...
function sjdg_css() {
    ...
    if ($logo) {
        ...
        $logo_max_width = get_theme_mod('sjdg_logo_width', 350);
    ...
    $css .= ( sjdg_customizer_get_default_link_color() !== $color_link ) ? sprintf(
                    '
    ...
    $css .= ( sjdg_customizer_get_default_accent_color() !== $color_accent ) ? sprintf(
                    '
		...
		', $color_accent, sjdg_color_contrast($color_accent)
            ) : '';
