 ...
 * Sant Joan de Gracia.
 *
 * This file adds the required WooCommerce setup functions to the Sant Joan de Gracia Theme.
 *
...
add_action( 'wp_enqueue_scripts', 'sjdg_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function sjdg_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'sjdg_woocommerce_breakpoint' );
/**
 ...
 */
function sjdg_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'sjdg_default_products_per_page' );
/**
 ...
 */
function sjdg_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'sjdg_woocommerce_pagination' );
/**
 ...
 */
function sjdg_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'sjdg' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'sjdg' ) );

	return $args;

}

add_action( 'after_switch_theme', 'sjdg_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function sjdg_woocommerce_image_dimensions_after_theme_setup() {
	...
	sjdg_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'sjdg_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function sjdg_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function sjdg_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'sjdg_gallery_image_thumbnail' );
/**
 ...
 */
function sjdg_gallery_image_thumbnail( $size ) {
